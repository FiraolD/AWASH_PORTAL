...
class PremiumCalculationService {
    ...
    async getBaseRate(productCode, coverageAmount) {
    ...
    async getRiskModifier(productCode, category, factorValue) {
        const result = await pool.query(`
      SELECT modifier
      FROM risk_factors
      WHERE "productCode" = $1
    ...
    async getAgeModifier(productCode, age) {
        const result = await pool.query(`
      SELECT modifier
      FROM risk_factors
      WHERE "productCode" = $1
    ...
    async getCylinderModifier(productCode, cylinderCapacity) {
        if (!cylinderCapacity)
            return 1.0;
        const result = await pool.query(`
      SELECT modifier
    ...
    async getMultiVehicleDiscount(vehicleCount) {
        const result = await pool.query(`
    ...
    async calculateVehiclePremium(productCode, vehicle, baseRate) {
        const adjustments = [];
    ...
    async calculatePremium(params) {
        ...
        if (isNaN(totalRiskModifier))
            totalRiskModifier = 1.0;
        return {
