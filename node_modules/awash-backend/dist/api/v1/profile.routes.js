...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT id, email, "firstName", "lastName", phone, "avatarUrl", address, "createdAt", "lastLoginAt", "updatedAt"
...
router.put('/', async (req, res) => {
    try {
       ...
       SET "firstName" = COALESCE($1, "firstName"),
           ...
           "updatedAt" = NOW()
       WHERE id = $5
       RETURNING id, email, "firstName", "lastName", phone, "avatarUrl", address, "createdAt", "updatedAt"`, [firstName, lastName, phone, address, userId]);
...
router.post('/avatar', uploadAvatar.single('avatar'), async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE users SET "avatarUrl" = $1, "updatedAt" = NOW() WHERE id = $2 RETURNING "avatarUrl"`, [avatarUrl, userId]);
