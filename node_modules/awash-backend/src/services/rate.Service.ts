...
let ratesCache: RateConfig[] | null = null;
let cacheTime: number = 0;
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes

async function loadRatesFromDB(): Promise<RateConfig[]> {
...
export async function getBaseRate(productType: string, coverageAmount: number): Promise<number> {
  ...
  const applicableRate = ratesCache.find(rate => 
...
export async function getRiskFactor(productType: string, coverageAmount: number, customerAge?: number): Promise<number> {
  let riskFactor = 1.0;
