...
class ApprovalService {
    ...
    async getApprovalFlowWithDetails(approvalFlowIds) {
        if (!approvalFlowIds.length)
            return [];
        const result = await pool.query(`SELECT * FROM role_levels WHERE id = ANY($1) AND is_active = true ORDER BY level_order ASC`, [approvalFlowIds]);
    ...
    async findMatchingRule(productType, sumInsured, riskScore = 0) {
        const result = await pool.query(`SELECT * FROM approval_rules
       ...
       LIMIT 1`, [productType, sumInsured, riskScore]);
    ...
    async requiresApproval(entityType, productType, sumInsured, riskScore = 0) {
    ...
    async createApprovalRequest(entityId, entityType, productType, sumInsured, riskScore, requestedBy, metadata = {}) {
        const approvalCheck = await this.requiresApproval(entityType, productType, sumInsured, riskScore);
        ...
        const insertResult = await pool.query(`INSERT INTO approval_requests
       ...
       RETURNING *`, [uuidv4(), requestNumber, entityType, entityId, requestedBy, firstLevel?.id || '', approvalMetadata]);
    ...
    async getPendingRequest(entityId, entityType) {
        const result = await pool.query(`SELECT * FROM approval_requests WHERE entity_id = $1 AND entity_type = $2 AND status = 'PENDING' ORDER BY submitted_at DESC LIMIT 1`, [entityId, entityType]);
        if (!result.rows.length)
            return null;
        return this.mapApprovalRequestRow(result.rows[0]);
    }
    async processApproval(requestId, approverId, approverRole, action, comments = '') {
        const requestResult = await pool.query(`SELECT * FROM approval_requests WHERE id = $1`, [requestId]);
        if (!requestResult.rows.length) {
            throw new Error('Approval request not found');
        }
        const request = requestResult.rows[0];
        const approvalFlow = (request.approval_metadata?.approval_flow || []);
        ...
        await pool.query(`INSERT INTO approval_history
       (id, request_id, approved_by, approval_level, decision, notes)
       VALUES ($1, $2, $3, $4, $5, $6)`, [uuidv4(), requestId, approverId, request.current_level, action, comments]);
        if (action === 'REJECTED') {
    ...
    async getUserPendingApprovals(userId, userRole, userDepartment) {
        const levelsResult = await pool.query(`SELECT * FROM role_levels WHERE is_active = true AND (level_code = $1 OR (department = $2 AND can_approve = true)) ORDER BY level_order ASC`, [userRole, userDepartment || null]);
        const userLevelIds = levelsResult.rows.map((row) => row.id);
        if (!userLevelIds.length)
            return [];
        const requests = await pool.query(`SELECT ar.*, u.first_name, u.last_name, u.email
       FROM approval_requests ar
       JOIN users u ON ar.requested_by = u.id
       WHERE ar.status = 'PENDING' AND ar.current_level = ANY($1)
       ORDER BY ar.submitted_at ASC`, [userLevelIds]);
        return await Promise.all(requests.rows.map(async (request) => {
    ...
    async getApprovalHistory(entityId, entityType) {
        const result = await pool.query(`SELECT ah.*, u.first_name, u.last_name, u.email
       ...
       ORDER BY ah.created_at ASC`, [entityId, entityType]);
    ...
    async isApproverAuthorized(approverRole, level, approverId) {
        if (!level)
            return false;
        if (approverRole === 'MASTER_ADMIN')
            return true;
    ...
    async autoApproveEntity(entityId, entityType, approvedBy, reason) {
        await this.finalizeApproval(entityId, entityType);
    }
    async finalizeApproval(entityId, entityType) {
        switch (entityType) {
            case 'POLICY':
    ...
    async notifyApprovers(requestId, level) {
        if (!level)
            return;
        const result = await pool.query(`SELECT ar.*, u.first_name, u.last_name, u.email
       FROM approval_requests ar
       JOIN users u ON ar.requested_by = u.id
       WHERE ar.id = $1`, [requestId]);
        ...
        const approvers = await pool.query(`SELECT id, email, first_name, last_name FROM users WHERE role = $1 AND status = 'ACTIVE'`, [level.levelCode]);
        approvers.rows.forEach((approver) => {
    ...
    async getApprovalStats(userId, userRole) {
        ...
        const approvedRequests = await pool.query(`SELECT submitted_at, updated_at FROM approval_requests WHERE status = 'APPROVED'`);
        const approvedDurations = approvedRequests.rows
            .filter((r) => r.submitted_at && r.updated_at)
            .map((r) => (new Date(r.updated_at).getTime() - new Date(r.submitted_at).getTime()) / 3600000);
        const avgApprovalHours = approvedDurations.length
            ? approvedDurations.reduce((sum, hours) => sum + hours, 0) / approvedDurations.length
            : 0;
        const pendingByLevelResult = await pool.query(`SELECT ar.current_level, rl.level_order, rl.level_name, rl.level_code, COUNT(*) as count
       FROM approval_requests ar
       ...
       ORDER BY rl.level_order ASC`);
        const recentRequestsResult = await pool.query(`SELECT ar.*, u.first_name, u.last_name, u.email,
       ...
       LIMIT 10`);
        const recentApprovals = recentRequestsResult.rows.map((request) => ({
            ...request,
            requester_name: `${request.first_name || ''} ${request.last_name || ''}`.trim(),
            requester_email: request.email,
        ...
        return {
            ...
            pendingByLevel: pendingByLevelResult.rows.map((row) => ({
    ...
    async getApprovalFlow(entityId, entityType) {
        const result = await pool.query(`SELECT ar.*, u.first_name, u.last_name, u.email
       FROM approval_requests ar
       JOIN users u ON ar.requested_by = u.id
       WHERE ar.entity_id = $1 AND ar.entity_type = $2 AND ar.status = 'PENDING'
       ORDER BY ar.submitted_at DESC
       LIMIT 1`, [entityId, entityType]);
        if (!result.rows.length) {
        ...
        const approvalFlow = (request.approval_metadata?.approval_flow || []);
        const currentLevelIndex = approvalFlow.findIndex((level) => level.id === request.current_level);
        const flow = approvalFlow.map((level, index) => ({
            id: level.id,
            levelCode: level.levelCode,
            levelName: level.levelName,
            department: level.department,
