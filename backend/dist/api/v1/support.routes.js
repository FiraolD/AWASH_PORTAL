...
router.get('/admin/tickets', authenticate, authorize('CUSTOMER_ADMIN', 'MASTER_ADMIN'), async (req, res) => {
    try {
        const query = `
            SELECT 
                t.id,
...
router.post('/tickets', authenticate, async (req, res) => {
    try {
        ...
        const { subject, message = req.body.description, priority = 'medium', category = 'general' } = req.body;
        if (!subject || !message) {
...
router.get('/stats', authenticate, authorize('CUSTOMER_ADMIN', 'MASTER_ADMIN'), async (req, res) => {
    try {
        const query = `
            SELECT 
