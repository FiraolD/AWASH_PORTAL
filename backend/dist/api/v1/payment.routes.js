...
async function generateReference() {
    ...
    const result = await pool.query(`SELECT reference FROM payment_references 
...
// ---------------------------------------------------------------------------
router.post('/generate-reference', authenticate, authorize('SUPERVISOR_UNDERWRITING', 'UNDERWRITING_MANAGER', 'HEAD_UNDERWRITING', 'UNDERWRITING_ADMIN', 'MANAGER_CLAIMS', 'HEAD_CLAIMS', 'CLAIMS_ADMIN', 'MASTER_ADMIN'), async (req, res) => {
    ...
    try {
        ...
        if (policyId) {
            const policyCheck = await client.query('SELECT * FROM policies WHERE id = $1', [policyId]);
        ...
        await client.query(`INSERT INTO payment_references (
        ...
        if (policyId) {
            await client.query(`UPDATE policies 
         SET status = 'PENDING_PAYMENT', 
             "paymentReference" = $1, 
             "updatedAt" = NOW() 
         WHERE id = $2`, [reference, policyId]);
            // Add timeline
            await client.query(`INSERT INTO policy_timeline (id, "policyId", status, note, "changedBy", "createdAt")
         VALUES (gen_random_uuid()::text, $1, 'PENDING_PAYMENT', $2, $3, NOW())`, [policyId, `Payment reference generated: ${reference} for ETB ${amount}`, userId]);
        }
...
// ---------------------------------------------------------------------------
router.get('/references', authenticate, authorize('SUPERVISOR_UNDERWRITING', 'UNDERWRITING_MANAGER', 'HEAD_UNDERWRITING', 'MANAGER_CLAIMS', 'HEAD_CLAIMS', 'CLAIMS_ADMIN', 'MASTER_ADMIN'), async (req, res) => {
    try {
...
router.get('/lookup/:reference', async (req, res) => {
    try {
       ...
       LEFT JOIN policies p ON p.id = pr."policyId"
       LEFT JOIN claims c ON c.id = pr."claimId"
       WHERE pr.reference = $1`, [reference]);
        if (result.rows.length === 0) {
...
// ---------------------------------------------------------------------------
router.get('/my-references', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const result = await pool.query(`SELECT pr.*, p."policyNumber", c."claimNumber"
       FROM payment_references pr
       LEFT JOIN policies p ON p.id = pr."policyId"
       LEFT JOIN claims c ON c.id = pr."claimId"
       WHERE p."userId" = $1 OR c."userId" = $1
       ORDER BY pr."createdAt" DESC`, [userId]);
        res.json(result.rows);
    }
    catch (error) {
...
router.post('/webhook/:provider', async (req, res) => {
    try {
        ...
        if (isSuccess) {
            await pool.query(`UPDATE payment_references 
         SET status = 'PAID', 
             "transactionId" = $1, 
             provider = $2,
             "paidAmount" = $3, 
             "paidAt" = NOW(), 
             "updatedAt" = NOW()
         WHERE reference = $4`, [transactionId, provider, paidAmount, reference]);
            // Update policy status
            const refResult = await pool.query('SELECT "policyId" FROM payment_references WHERE reference = $1', [reference]);
            if (refResult.rows.length > 0 && refResult.rows[0].policyId) {
                await pool.query(`UPDATE policies SET status = 'PAYMENT_RECEIVED', "updatedAt" = NOW() WHERE id = $1`, [refResult.rows[0].policyId]);
            }
        }
