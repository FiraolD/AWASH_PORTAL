...
router.get('/hospitals', async (req, res) => {
    try {
        const result = await pool.query(`SELECT "hospitalName" 
       FROM hospital_list 
       WHERE "isActive" = true 
       ORDER BY "hospitalName" ASC`);
        res.json(result.rows.map((row) => row.hospitalName));
    }
    catch (error) {
...
// ---------------------------------------------------------------------------
router.post('/hospitals', authenticate, authorizeExecutives, async (req, res) => {
    try {
        const { name } = req.body;
        if (!name || name.trim().length === 0) {
            return res.status(400).json({ error: 'Hospital name is required' });
        }
        await pool.query(`INSERT INTO hospital_list ("hospitalName") VALUES ($1) ON CONFLICT ("hospitalName") DO NOTHING`, [name.trim()]);
...
// ---------------------------------------------------------------------------
router.put('/hospitals/:name', authenticate, authorizeExecutives, async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE hospital_list SET "hospitalName" = $1 WHERE "hospitalName" = $2`, [newName.trim(), oldName]);
...
// ---------------------------------------------------------------------------
router.delete('/hospitals/:name', authenticate, authorizeExecutives, async (req, res) => {
    try {
...
router.get('/', authenticate, authorizeExecutives, async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, "settingKey", "settingValue", "settingType", description, "isPublic", "createdAt", "updatedAt", "updatedBy"
...
router.put('/:key', authenticate, authorizeExecutives, async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE system_settings 
       SET "settingValue" = $1, "updatedAt" = NOW(), "updatedBy" = $2
       WHERE "settingKey" = $3`, [value, userId, key]);
