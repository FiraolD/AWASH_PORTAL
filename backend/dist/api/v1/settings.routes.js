...
router.get('/', authenticate, authorize(...SETTINGS_ROLES), async (req, res) => {
    try {
        const result = await pool.query(`SELECT setting_key, setting_value, description, updated_at, updated_by 
...
router.get('/backup', authenticate, authorize(...SETTINGS_ROLES), async (req, res) => {
    try {
        const result = await pool.query(`SELECT setting_value FROM system_settings WHERE setting_key = 'backupSettings'`);
...
router.put('/backup', authenticate, authorize(...SETTINGS_ROLES), async (req, res) => {
    ...
    try {
       ...
       SET setting_value = EXCLUDED.setting_value,
           updated_at = NOW(),
           updated_by = EXCLUDED.updated_by`, [JSON.stringify(backupSettings), userId]);
...
router.put('/maintenance', authenticate, authorize(...SETTINGS_ROLES), async (req, res) => {
    ...
    try {
       ...
       SET setting_value = EXCLUDED.setting_value,
           updated_at = NOW(),
           updated_by = EXCLUDED.updated_by`, [JSON.stringify(maintenanceMode), userId]);
...
router.get('/hospitals', async (req, res) => {
    try {
        const result = await pool.query(`SELECT hospital_name 
       FROM hospital_list 
...
router.get('/public', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT setting_key, setting_value FROM system_settings WHERE setting_key = ANY($1)`, [publicKeys]);
        const settings = {};
        for (const row of result.rows) {
            try {
                settings[row.setting_key] = JSON.parse(row.setting_value);
