...
router.get('/public', async (req, res) => {
    try {
        const result = await pool.query(`SELECT "settingKey", "settingValue", "settingType" FROM system_settings WHERE "isPublic"=true ORDER BY "settingKey"`);
        const settings = {};
...
router.put('/:key', authenticate, authorizeExecutives, async (req, res) => {
    try {
       ...
       RETURNING *`, [key, String(value), type || 'string', description || null, isPublic !== undefined ? isPublic : false, req.user.id]);
