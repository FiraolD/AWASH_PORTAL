...
router.get('/stats', authenticate, async (req, res) => {
    try {
        ...
        // CUSTOMER Dashboard
            ...
            const activePolicies = await pool.query("SELECT COUNT(*) as count FROM policies WHERE \"userId\" = $1 AND status = 'ACTIVE'", [userId]);
            const pendingClaims = await pool.query("SELECT COUNT(*) as count FROM claims WHERE \"userId\" = $1 AND status IN ('SUBMITTED', 'UNDER_REVIEW')", [userId]);
            const openTickets = await pool.query("SELECT COUNT(*) as count FROM support_tickets WHERE \"userId\" = $1 AND status IN ('OPEN', 'IN_PROGRESS')", [userId]);
            const totalProducts = await pool.query("SELECT COUNT(*) as count FROM products where \"userID\" = $1 AND isActive = 'true'", [userId]);
            res.json({
                activePolicies: parseInt(activePolicies.rows[0].count),
                pendingClaims: parseInt(pendingClaims.rows[0].count),
        ...
        // UNDERWRITING Department Dashboard
            ...
            const pendingReviews = await pool.query("SELECT COUNT(*) as count FROM policies WHERE status IN ('PENDING_UNDERWRITING', 'SUBMITTED')");
            const pendingFinalApprovals = await pool.query("SELECT COUNT(*) as count FROM policies WHERE status = 'PENDING_FINAL_APPROVAL'");
            const totalPolicies = await pool.query("SELECT COUNT(*) as count FROM policies");
            res.json({
