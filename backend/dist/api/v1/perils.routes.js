...
router.post('/', authenticate, authorize('MASTER_ADMIN'), async (req, res) => {
    try {
        const { productId, perilName, description, premiumRate, calculationType, minCoverage, maxCoverage, isDefault, isOptional, displayOrder, isActive } = req.body;
        const result = await pool.query(`
      INSERT INTO perils (
...
router.put('/:id', authenticate, authorize('MASTER_ADMIN'), async (req, res) => {
    try {
        const { perilName, description, premiumRate, calculationType, minCoverage, maxCoverage, isDefault, isOptional, displayOrder, isActive } = req.body;
        const result = await pool.query(`
      UPDATE perils 
