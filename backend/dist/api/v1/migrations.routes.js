...
// ---------------------------------------------------------------------------
export async function recordMigration(migrationName) {
    try {
        await pool.query(`INSERT INTO migrations ("migrationName", "executedAt")
       VALUES ($1, NOW())
       ON CONFLICT ("migrationName") DO NOTHING`, [migrationName]);
...
// ---------------------------------------------------------------------------
export async function hasMigration(migrationName) {
    try {
        const result = await pool.query(`SELECT id FROM migrations WHERE "migrationName" = $1`, [migrationName]);
...
router.get('/status', authenticate, authorizeExecutives, async (req, res) => {
    try {
        ...
        const latestResult = await pool.query('SELECT "migrationName", "executedAt" FROM migrations ORDER BY "executedAt" DESC LIMIT 1');
        res.json({
