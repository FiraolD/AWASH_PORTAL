...
router.get('/', async (req, res) => {
    try {
        const result = await pool.query(`SELECT "hospitalName" FROM hospital_list WHERE "isActive"=true ORDER BY "hospitalName"`);
        res.json(result.rows.map((r) => r.hospitalName));
    }
    catch (error) {
...
router.post('/', authenticate, authorizeExecutives, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO hospital_list ("hospitalName", "isActive", "createdAt", "updatedAt") VALUES ($1,true,NOW(),NOW()) RETURNING *`, [hospitalName.trim()]);
