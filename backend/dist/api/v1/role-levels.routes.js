...
router.get('/', authenticate, authorizeExecutives, async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, "roleName", "displayName", "hierarchyLevel", department, "canApprove", "canReject", "canReview", "maxApprovalAmount", "isActive", "createdAt", "updatedAt"
...
router.get('/active/:department', authenticate, async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, "roleName", "displayName", "hierarchyLevel", "canApprove", "canReject", "canReview", "maxApprovalAmount"
       FROM role_levels WHERE department=$1 AND "isActive"=true ORDER BY "hierarchyLevel"`, [req.params.department]);
        res.json(result.rows);
    }
    catch (error) {
...
router.put('/:id', authenticate, authorizeExecutives, async (req, res) => {
    try {
        ...
        const d = req.body;
        await pool.query(`UPDATE role_levels SET "displayName"=$1,"hierarchyLevel"=$2,department=$3,"canApprove"=$4,"canReject"=$5,"canReview"=$6,"maxApprovalAmount"=$7,"isActive"=$8,"updatedAt"=NOW() WHERE id=$9`, [d.displayName !== undefined ? d.displayName : oldData.displayName, d.hierarchyLevel !== undefined ? d.hierarchyLevel : oldData.hierarchyLevel, d.department !== undefined ? d.department : oldData.department, d.canApprove !== undefined ? d.canApprove : oldData.canApprove, d.canReject !== undefined ? d.canReject : oldData.canReject, d.canReview !== undefined ? d.canReview : oldData.canReview, d.maxApprovalAmount !== undefined ? d.maxApprovalAmount : oldData.maxApprovalAmount, d.isActive !== undefined ? d.isActive : oldData.isActive, id]);
