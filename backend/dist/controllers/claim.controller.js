...
export const ClaimController = {
    ...
    getMyClaims: async (req, res) => {
        try {
            ...
            const result = await pool.query(`SELECT c.id, c."claimNumber", c.status, c."incidentDate", c."incidentDescription",
    ...
    getClaimQueue: async (req, res) => {
        try {
            const result = await pool.query(`SELECT c.id, c."claimNumber", c.status, c."submittedDate", c."estimatedAmount",
    ...
    getMyAssignedClaims: async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    getPendingReview: async (req, res) => {
        try {
            const result = await pool.query(`SELECT c.id, c."claimNumber", c.status, c."incidentDate", c."estimatedAmount",
                c."submittedDate", c."incidentDescription", c."natureOfLoss",
                u."firstName", u."lastName", u.email, u.phone,
    ...
    createClaim: async (req, res) => {
        try {
            ...
            const policyCheck = await pool.query(`SELECT id, type, "coverageAmount", "policyNumber"
         FROM policies
         WHERE id = $1 AND "userId" = $2`, [policyId, userId]);
            ...
            // Insert claim
            const insertResult = await pool.query(`INSERT INTO claims (
    ...
    updateClaimStatus: async (req, res) => {
        try {
           ...
           VALUES (gen_random_uuid(), $1, NOW(), $2, $3, NOW())`, [id, status, notes]);
            ...
            if (userResult.rows.length > 0) {
                const customer = userResult.rows[0];
                await EmailService.sendStatusUpdateEmail(customer.email, `${customer.firstName} ${customer.lastName}`, 'Claim', claimResult.rows[0].claimNumber, status);
            }
