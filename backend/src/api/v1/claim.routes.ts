...
router.get('/my-assigned', authenticate, async (req: any, res: any) => {
  try {
...
router.get('/queue', authenticate, authorize(...CLAIM_ROLES), async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT 
        c.id, c."claimNumber", c.status, c."incidentDate", c."estimatedAmount",
...
router.get('/queue/stats', authenticate, authorize(...CLAIM_ROLES), async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT 
...
router.get('/active', authenticate, authorize(...CLAIM_ROLES), async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT 
        c.id, c."claimNumber", c.status, c."incidentDate", c."estimatedAmount",
...
router.get('/pending-review', authenticate, authorize(...CLAIM_ROLES), async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT 
        c.id, c."claimNumber", c.status, c."incidentDate", c."estimatedAmount",
...
router.get('/stats/summary', authenticate, authorize(...ALL_CLAIMS_ACCESS), async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT 
        COUNT(*) as total,
...
router.get('/team-performance', authenticate, authorize('MANAGER_CLAIMS', 'HEAD_CLAIMS', 'CLAIMS_ADMIN', 'MASTER_ADMIN'), async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT 
...
router.get('/recent-activities', authenticate, authorize('MANAGER_CLAIMS', 'HEAD_CLAIMS', 'CLAIMS_ADMIN', 'MASTER_ADMIN'), async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT 
...
router.get('/search', authenticate, authorize(...ALL_CLAIMS_ACCESS), async (req: any, res: any) => {
  try {
...
router.get('/my-claims', authenticate, async (req: any, res: any) => {
  try {
    const userId = req.user.id;
    const result = await pool.query(
      `SELECT 
...
router.post('/', authenticate, async (req: any, res: any) => {
  try {
    const userId = req.user?.id;
    const {
      policyId, productCode = 'GEN', riskItem, incidentDate, timeOfAccident,
      incidentDescription, location, estimatedAmount, natureOfLoss,
      ...
      weatherConditions, responsibleParty
    } = req.body;
    
    if (!policyId) return res.status(400).json({ error: 'Policy ID is required' });
    if (!incidentDate) return res.status(400).json({ error: 'Incident date is required' });
    if (!incidentDescription) return res.status(400).json({ error: 'Incident description is required' });
    if (!natureOfLoss) return res.status(400).json({ error: 'Nature of loss is required' });
    ...
    if (injuredPersons) {
      if (typeof injuredPersons === 'string') {
        try { JSON.parse(injuredPersons); injuredPersonsJson = injuredPersons; }
        catch (e) { injuredPersonsJson = JSON.stringify([{ name: injuredPersons }]); }
...
router.post('/:id/review', authenticate, authorize(...CLAIM_ROLES), validateBody(reviewSchema), async (req: any, res: any) => {
  ...
  try {
    ...
    await client.query(
      `UPDATE claims SET ${updateFields.join(', ')} WHERE id = $${paramCount}`,
      updateValues
    );
    ...
    await client.query(
      `INSERT INTO claim_timeline ("claimId", status, note, "changedBy", "createdAt")
       VALUES ($1, $2, $3, $4, NOW())`,
      [id, newStatus, timelineNote, userId]
    );
  ...
  try {
    ...
    for (const file of files) {
      const result = await pool.query(
        ...
        [id, file.originalname, file.path, file.mimetype || 'application/octet-stream', userId, file.size]
      );
...
router.get('/:id/documents', authenticate, async (req: any, res: any) => {
  try {
    const userId = req.user?.id;
    const userRole = req.user?.role;
    ...
    const result = await pool.query(
      `SELECT id, "documentName", "documentUrl", "documentType", "fileSize", "uploadedAt", "createdAt"
       FROM claim_documents WHERE "claimId" = $1 ORDER BY "createdAt" DESC`,
      [claimId]
    );
    res.json(result.rows);
  } catch (error) {
...
router.get('/:id/documents/:documentId/download', authenticate, async (req: any, res: any) => {
  try {
    const userId = req.user?.id;
    const userRole = req.user?.role;
    ...
    if (documentResult.rows.length === 0) return res.status(404).json({ error: 'Document not found' });
    ...
    if (!allowedExtensions.includes(ext)) return res.status(403).json({ error: 'File type not allowed for download' });
...
router.get('/:id', authenticate, async (req: any, res: any) => {
  try {
    ...
    if (claimData.productDetails) {
      ...
      } catch (e) { console.error('Error parsing productDetails:', e); }
    }
