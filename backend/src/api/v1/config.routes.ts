...
router.get('/', authenticate, authorize('MASTER_ADMIN'), async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM configuration ORDER BY category ASC, config_key ASC`
    );
...
router.get('/:category', authenticate, authorize('MASTER_ADMIN'), async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM configuration WHERE category = $1 ORDER BY config_key ASC`,
      [req.params.category]
    );
    res.json(result.rows);
  } catch (error) {
...
router.get('/role-levels/list', authenticate, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM role_levels WHERE is_active = true 
