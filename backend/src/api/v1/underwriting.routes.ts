...
router.post('/policies/:id/adjust',
  authorize(...UNDERWRITING_ROLES),
  async (req, res) => {
    try {
...
router.post('/policies/:id/final-approve',
  authorize('UNDERWRITING_ADMIN', 'MANAGER_UNDERWRITING', 'HEAD_UNDERWRITING', 'SENIOR_UNDERWRITING_OFFICER', 'MASTER_ADMIN'),
  async (req, res) => {
    try {
      ...
      // Get user details
      const userResult = await pool.query(
        'SELECT "firstName", "lastName", email, phone, address FROM users WHERE id = $1',
        [policy.userId]
      );
      ...
      try {
        ...
        const lastAdjustment = history.find((h: any) => h.action === 'PREMIUM_ADJUSTMENT');
        if (lastAdjustment) {
...
router.get('/pending-final-approval',
  authorize('UNDERWRITING_ADMIN', 'MANAGER_UNDERWRITING', 'HEAD_UNDERWRITING', 'SENIOR_UNDERWRITING_OFFICER', 'MASTER_ADMIN'),
  async (req, res) => {
    try {
...
router.post('/policies/:id/final-approve',
  ...
  async (req, res) => {
    try {
      ...
      const checkPolicy = await pool.query(
        'SELECT id, status, "policyNumber" FROM policies WHERE id = $1',
        [policyId]
      );
...
// Get head stats
router.get('/head/stats',
  authorize('HEAD_UNDERWRITING', 'UNDERWRITING_ADMIN', 'MASTER_ADMIN'),
  async (req, res) => {
    try {
...
// Get monthly trends
router.get('/head/monthly-trends',
  authorize('HEAD_UNDERWRITING', 'UNDERWRITING_ADMIN', 'MASTER_ADMIN'),
  async (req, res) => {
    try {
      const { year = new Date().getFullYear() } = req.query;
...
router.post('/endorsements/:id/approve',
  authorize('UNDERWRITING_ADMIN', 'MANAGER_UNDERWRITING', 'HEAD_UNDERWRITING', 'SENIOR_UNDERWRITING_OFFICER', 'MASTER_ADMIN'),
  async (req, res) => {
    try {
      const { notes } = req.body;
...
router.post('/endorsements/:id/reject',
  authorize('UNDERWRITING_ADMIN', 'MANAGER_UNDERWRITING', 'HEAD_UNDERWRITING', 'SENIOR_UNDERWRITING_OFFICER', 'MASTER_ADMIN'),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      const pendingFinalApprovals = await pool.query(
        `SELECT COUNT(*) FROM policies WHERE status = 'PENDING_FINAL_APPROVAL'`
      );
      
      // Pending endorsements
      let pendingEndorsements = { rows: [{ count: '0' }] };
      try {
        pendingEndorsements = await pool.query(
          `SELECT COUNT(*) FROM endorsements WHERE status = 'PENDING'`
        );
      } catch (err) {
      ...
      const totalActivePolicies = await pool.query(
        `SELECT COUNT(*) FROM policies WHERE status = 'ACTIVE'`
      );
      
      // Rejected policies
      const rejectedPolicies = await pool.query(
        `SELECT COUNT(*) FROM policies WHERE status IN ('REJECTED', 'REJECTED_BY_CUSTOMER')`
      );
