...
router.post('/calculate-premium', async (req, res) => {
  try {
    ...
    try {
      ...
      `, [productType.toUpperCase(), coverageAmount]);
      if (rateResult.rows.length > 0) baseRate = parseFloat(rateResult.rows[0].baseRate);
    } catch (err) { console.error('Error fetching base rate:', err); }
    
    let totalVehicleValue = 0;
    for (const vehicle of vehicles) totalVehicleValue += vehicle.vehicleValue || 0;
