...
router.post('/', authenticate, authorize('MASTER_ADMIN'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [email.toLowerCase(), hashedPassword, firstName, lastName, role || 'CUSTOMER', phone || null]
    );
    ...
    await pool.query(
      `UPDATE users 
       SET "firstName" = COALESCE($1, "firstName"),
           ...
           "updatedAt" = NOW()
       WHERE id = $6`,
      [firstName, lastName, role, phone, status, req.params.id]
    );
...
router.put('/customers/:id', authenticate, authorize('CUSTOMER_ADMIN', 'MASTER_ADMIN'), async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [firstName, lastName, phone, status, address, req.params.id]
    );
...
router.get('/stats/customers', authenticate, authorize('CUSTOMER_ADMIN', 'MASTER_ADMIN'), async (req, res) => {
  try {
    ...
    const newCustomersThisMonth = await pool.query(
      "SELECT COUNT(*) FROM users WHERE role = 'CUSTOMER' AND \"createdAt\" >= $1",
      [startOfMonth]
    );
