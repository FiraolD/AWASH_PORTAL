...
exports.login = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id, email, "passwordHash", role, "firstName", "lastName", "isActive" FROM users WHERE email = $1',
...
exports.getProfile = async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, email, role, "firstName", "lastName", phone, address, "createdAt" FROM users WHERE id = $1',
      [req.user.id]
    );
