...
export default function ClaimsAdminDashboard() {
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      toast.error('Unable to load the admin dashboard right now');
    } finally {
  ...
  if (loading) {
    return <div className="flex min-h-[320px] items-center justify-center text-sm text-gray-500">Loading administration dashboard...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#1A3E6F]">Claims Administration Dashboard</h1>
          <p className="text-sm text-gray-500">Live system-wide claims monitoring and operations</p>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {filteredQueue.length === 0 ? (
              <p className="py-6 text-center text-sm text-gray-500">No claims in the queue.</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {teamMembers.length === 0 ? (
                <p className="py-6 text-center text-sm text-gray-500">No team data yet.</p>
